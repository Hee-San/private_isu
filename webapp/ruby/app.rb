...
module Isuconp
  ...
  class App < Sinatra::Base
    ...
    helpers do
      ...
      def make_posts(results, all_comments: false)
        ...
        results.to_a.each do |post|
          ...
          # 削除されていないuserの場合、postsにpush
          # TODO: selectで除けろ
          # before: posts.push(post) if post[:user][:del_flg] == 0
          posts.push(post)
          # resultsが20件しかいらない
    ...
    get '/' do
      ...
      results = db.query('SELECT `id`, `user_id`, `body`, `created_at`, `mime` FROM `posts` p  JOIN users u ON p.user_id = u.id WHERE u.del_flg = 0 ORDER BY `created_at` DESC')
